...
def enter_random_request():
    ...
    for x in range(10):
        random_requst = random_requst + random.choice(list('12345678909qwertyuiopasdfghjklzxcvbnmQWERTYUIOPASDFGHJKLKZXZCVBNM'))
...
def test_search_negative(set_browser_configuration):
    browser.element('[name="q"]').should(be.blank).type(enter_random_request()).press_enter()
    assert browser.element('[id="search"]').should(have.text('Selene - User-oriented Web UI browser tests in Python'))
